# -*- coding: utf-8 -*-
"""
Created on Sat Apr 20 14:42:51 2019
...
class poker:#扑克的一个类，下面使用的变量如果在两个实例方法里使用，要定义成实例属性。要不然会出现后面不能引用前面的变量。
    ...
    def yingshe(self):#映射
        self.paizd=[(0,'方片4'),(1,'梅花4'),(2,'红桃4'),(3,'黑桃4'),
        ...
        self.zdpai = dict(paizd)
        paistr1=''
        for i in range (len(self.str1)):
            paistr1+=self.zdpai[self.str1[i]]+' '
        paistr2=''
        for i in range (len(self.str2)):
            paistr2+=self.zdpai[self.str2[i]]+' '
        paistr3=''
        for i in range (len(self.str3)):
            paistr3+=self.zdpai[self.str3[i]]+' '
